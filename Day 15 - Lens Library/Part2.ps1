...
1. If there is already a lens in the box with the same label, replace the old
   lens with the new lens: remove the old lens and put the new lens in its
   place, not moving any other lenses in the box.

2. If there is not already a lens in the box with the same label, add the lens
...
$Steps = $InputData -split ','

# Create the 256 boxes
$Boxes = @{}
for ($i = 0; $i -lt 256; $i++) {
    $Boxes.Add($i, [System.Collections.ArrayList]::new())
}

foreach ($Step in $Steps) {
    $OpAt = $Step.IndexOfAny('=+-')

    $Letters = $Step.Substring(0, $OpAt)
    $Operation = $Step[$OpAt]
    $Operand = $Step.Substring($OpAt + 1)
    $Box = HASH -Text $Letters
    if ($Operation -eq '=') {
        # check if the box already contains a lens with the same label
        $Lens = $Boxes[$Box].Where({ $_.Label -eq $Letters })[0]
        if ($Lens) {
            # replace the old lens with the new lens
            try {
                $Lens.FocalLength = [int32]$Operand
            } catch {
                Write-Host "Uh oh!"
            }
        } else {
            ...
            $Lens = [PSCustomObject]@{
                Label = $Letters
                FocalLength = [int]$Operand
            }
            $Boxes[$Box].Add($Lens) | Out-Null
        }
    } elseif ($Operation -eq '-') {
        # remove the lens with the given label if it is present in the box
        $Lens = $Boxes[$Box].Where({$_.Label -eq $Letters })[0]
        if ($Lens) {
            $Boxes[$Box].Remove($Lens) | Out-Null
        }
    } else {
        throw "Unknown isntruction: $Step"
    }
}
...
foreach ($Box in $Boxes.Keys) {
    ...
    foreach ($Lens in $Boxes[$Box]) {
        $FocusingPower += ($Box + 1) * ($Boxes[$Box].IndexOf($Lens) + 1) * $Lens.FocalLength
    }
}
