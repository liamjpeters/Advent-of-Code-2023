...
# Create an ordered dictionary to store the cards in.
# The order is important so we process the cards correctly.
$Cards = [Ordered]@{}

# Loop over the cards and parse them, including matching numbers
foreach ($Card in $InputData) {
    # First split the cards into the card ID bit and the numbers bit
    $CardID, $CardNumbers = $Card -split ': '
    
    $Id = [int64]($CardID -replace 'Card ')

    # Next split the numbers into the winning numbers and elf numbers
    $WinningNumbers, $ElfNumbers = $CardNumbers -split ' \| '

    # Convert the string array of 'numbers' into integers
    $WinningNumbers = $WinningNumbers -split ' ' | ForEach-Object {
        if (-not [string]::IsNullOrWhiteSpace($_)){
            [int64]$_
        }
    }
    $ElfNumbers = $ElfNumbers -split ' ' | ForEach-Object {
        if (-not [string]::IsNullOrWhiteSpace($_)){
            [int64]$_
        }
    }

    # Compare the 2 lists of numbers. We only care about the total number of
    # matches.
    $CompareParams = @{
        ReferenceObject = @($WinningNumbers)
        DifferenceObject = @($ElfNumbers)
        IncludeEqual = $true
        ExcludeDifferent = $true
        PassThru = $true
    }
    $NumberOfMatches = Compare-Object @CompareParams | Measure-Object | 
...
foreach ($Card in $Cards.GetEnumerator()) {
    # For each copy of the card we have
    for ($i = 0; $i -lt $Card.Value.NumberOfCopies; $i++) {
        # Add in copies of the next X cards, where X is the number of matching
        # numbers on the card
        for ($j = 1; $j -le $Card.Value.NumberOfMatches; $j++) {
            # Ensure we don't try to add a copy of a card that doesn't exist
            # in the deck
            if ($null -ne $Cards[$Card.Name + $j]) {
                $Cards[$Card.Name + $j].NumberOfCopies++
            }
        }
    }
}
...
$Cards.GetEnumerator() | 
    ForEach-Object { $_.Value.NumberOfCopies } | 
    Measure-Object -Sum | 
