...
# Start with a total of 0 points
$TotalPoints = 0

# Loop over the cards 1 at a time
foreach ($Card in $InputData) {
    ...
    # Work out the points
    $Points = if ($NumberOfMatches -eq 1) {
        1
    } elseif ($NumberOfMatches -gt 1) {
        [math]::Pow(2, $NumberOfMatches - 1)
    } else {
        0
    }

    # Add any points to the totals
    $TotalPoints += $Points
}
